...
namespace AKSaigyouji.AtlasGeneration
{
    ...
    public sealed class ChartProviderAutoConnect : ChartProviderModule
    {
        ...
        [Tooltip("A cell in the maze marked with this string will take a chart from the start charts.")]
        [SerializeField] string entrance = "Entrance";

        [Tooltip("A cell in the maze marked with this string will take a chart from the exit charts.")]
        ...
        ChartStatic[] DetermineChartCandidates(Maze maze, Coord cell)
        {
            ...
            {
                MetaData cellInfo = maze.GetInfo(cell);
                if (cellInfo.ContainsKey(entrance))
                {
                    return entranceCharts;
                }
                else if (cellInfo.ContainsKey(exit))
                {
                    return exitCharts;
                }
            }
